...
const PoemCard = ({ poem, onLike, updatePoem, currentUser, onDelete }) => {
  ...
  const handleLike = async () => {
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/poems/${poem._id}/like`,
        {},
        {
          headers: {
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/poems/${poem._id}/comment`,
        { text: comment },
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('authToken')}`,
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      const response = await axios.delete(
        `${process.env.REACT_APP_API_URL}/api/poems/${poem._id}/comments/${commentId}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('authToken')}`,
          },
  ...
  const handleUpdate = async () => {
    try {
      const response = await axios.put(
        `${process.env.REACT_APP_API_URL}/api/poems/${poem._id}`,
